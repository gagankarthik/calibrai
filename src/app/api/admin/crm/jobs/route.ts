  ...
  try {
    ...
    if (source) {
      items = items.filter((j) =>
        String(j.source ?? '').toLowerCase().includes(source.toLowerCase()),
      )
    }
    ...
    console.error('[admin/crm/jobs] error:', err)
    return NextResponse.json({ error: 'Failed to fetch CRM jobs' }, { status: 500 })
  }
}
