...
const FOOTER_COLS = {
  Product:   ['AI Matching', 'Discovery', 'Pipeline', 'Analytics'],
  Company:   ['About', 'Blog', 'Careers', 'Press'],
  Resources: ['Docs', 'Help', 'Security', 'Status'],
          ...
          {Object.entries(FOOTER_COLS).map(([group, links]) => (
            <div key={group}>
              ...
              <ul className="space-y-2.5">
                {links.map(l => (
                  <li key={l}>
                    <Link href="#" className="text-[13px] text-tl-text-tertiary hover:text-tl-text-primary transition-colors">
                      {l}
                    </Link>
                  </li>
                ))}
